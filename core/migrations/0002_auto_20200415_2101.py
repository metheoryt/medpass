...
def default_countries(apps, schema_editor):
    ...
    for id, name in (
        ...
        Country.objects.filter(pk=85).update(priority=40)  # Казахстан
        Country.objects.filter(pk=166).update(priority=30)  # РФ
        Country.objects.filter(id=1209).update(priority=29)  # Украина
        Country.objects.filter(id__in=OTHER_CIA).update(priority=20)  # СНГ
        Country.objects.filter(pk=1085).update(priority=19)  # Казахстан legacy id (или наоборот)
        Country.objects.filter(pk=1166).update(priority=19)  # РФ legacy id (или наоборот)
